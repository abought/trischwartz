...
/**
 * Scale and translate coordinates to fit inside a unit square:
 * origin at 0,0
 * @param coords
 */
function scaleCoords(coords) {
    ...
    // To preserve perspective, apply the larger transform
    var scale = 1 / Math.max(xr, yr);

    var xshift = xmin * scale;
    var yshift = ymin * scale;
    console.log(xr,yr, scale, xshift);
    return coords.map(function(item) {
        return [
            (item[0] * scale - xshift).toFixed(5),
            (item[0] * scale - yshift).toFixed(5)
        ];
    });
}
